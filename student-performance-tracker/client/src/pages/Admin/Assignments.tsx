...
const AssignmentManager = () => {
	...
	useEffect(() => {
		const fetchSubjects = async () => {
			...
			try { const response = await api.get(`/subjects/filter?course=${encodeURIComponent(form.course)}&semester=${encodeURIComponent(form.semester)}`); const subjectsData = Array.isArray(response) ? response : []; setSubjects(subjectsData); } catch (error: any) { setErrorMsg('Failed to load subjects. Please try again.'); setSubjects([]); }
		};
		fetchSubjects();
	...
	const handleSubmit = async (e: React.FormEvent) => {
		...
		setLoading(true);
		const payload = { course: form.course.trim(), semester: form.semester.trim(), subject: form.subject.trim(), title: form.title.trim(), dueDate: form.dueDate.trim(), teacherName: form.teacherName.trim(), teacherId: form.teacherId.trim() };
		try {
			if (isEditing && editId) { await api.put(`/assignments/${editId}`, payload); setAssignments((prev) => prev.map((a) => (a._id === editId || a.id === editId ? { ...a, ...payload, _id: editId, id: editId } : a))); }
			else { const saved = await api.post('/assignments', payload); setAssignments((prev) => [...prev, saved]); }
			setForm({ course: '', semester: '', subject: '', title: '', dueDate: '', teacherName: '', teacherId: '' }); setIsEditing(false); setEditId(null);
		} catch (error: any) { setErrorMsg(error?.message || 'Failed to save assignment.'); } finally { setLoading(false); }
	};

	const handleEdit = (assignment: any) => { setForm({ course: assignment.course || '', semester: assignment.semester?.toString() || '', subject: assignment.subject || '', title: assignment.title || '', dueDate: assignment.dueDate ? assignment.dueDate.substring(0,10) : '', teacherName: assignment.teacherName || assignment.teacher || '', teacherId: assignment.teacherId || '' }); setEditId(assignment._id || assignment.id); setIsEditing(true); };
	const handleDelete = async (id: string) => { if (!window.confirm('Are you sure you want to delete this assignment?')) return; await api.delete(`/assignments/${id}`); setAssignments((prev) => prev.filter((a) => a._id !== id && a.id !== id)); if (editId === id) { setIsEditing(false); setEditId(null); setForm({ course: '', semester: '', subject: '', title: '', dueDate: '', teacherName: '', teacherId: '' }); } };
	const handleCancelEdit = () => { setForm({ course: '', semester: '', subject: '', title: '', dueDate: '', teacherName: '', teacherId: '' }); setIsEditing(false); setEditId(null); setErrorMsg(''); };
