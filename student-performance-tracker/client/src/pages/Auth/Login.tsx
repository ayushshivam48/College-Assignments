...
const Login = () => {
	...
	const handleSubmit = async (e: React.FormEvent) => {
		...
		setLoading(true);
		try {
			const loginPayload: any = { password: password.trim(), role };
			...
			if (error.response) {
				...
				if (status === 429) setErrorMsg("Too many login attempts. Please try again later.");
				else if (data && data.message) setErrorMsg(data.message);
				else setErrorMsg("Login failed. Please check your credentials and try again.");
			} else if (error.request) setErrorMsg("Network error. Please check your connection and try again.");
			else setErrorMsg("An unexpected error occurred. Please try again.");
