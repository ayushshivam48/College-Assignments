			...
			<header className="w-full fixed top-0 bg-white/80 backdrop-blur-lg z-50 transition-all border-b border-white/20 shadow-lg">
				<div className="max-w-7xl mx-auto px-6 py-4 flex justify-between items-center">
					...
					<div className="md:hidden relative text-black">
						...
						>
							<a href="#about" className="block py-2 text-sm hover:text-blue-700">
								About
							</a>
							<a href="#features" className="block py-2 text-sm hover:text-blue-700">
								Features
							</a>
							<a href="#roles" className="block py-2 text-sm hover:text-blue-700">
								Roles
							</a>
							<Link
								to="/login"
			...
			>
				...
				>
					...
					<div className="flex flex-col sm:flex-row gap-4 justify-center items-center">
						...
						<a
							href="#features"
							className="px-8 py-4 rounded-full font-semibold text-lg border border-blue-300 hover:bg-blue-100 transition-all"
						>
							Learn More →
						</a>
					</div>
				...
				<div className="max-w-6xl mx-auto relative">
					...
					<div className="grid md:grid-cols-3 gap-8">
						{[
							...
							>
								<div
									className={`w-14 h-14 mb-6 rounded-xl bg-${(feature as any).color}-100 flex items-center justify-center text-2xl group-hover:scale-110 transition-transform`}
								>
									{(feature as any).icon}
								</div>
								<h4
									className={`text-xl font-semibold text-${(feature as any).color}-600 mb-3`}
								>
									{(feature as any).title}
								</h4>
								<p className="text-gray-700 leading-relaxed">{(feature as any).text}</p>
							</div>
						))}
					</div>
				</div>
						...
						{[
							{
								...
								role: "Students",
								text:
									"View grades, analyze subject performance, download reports, and read teacher feedback – all in one place.",
							...
							},
							{
								...
								role: "Teachers",
								text:
									"Upload marks, monitor class performance, provide student feedback, and generate visual insights effortlessly.",
							...
							},
							{
								...
								role: "Admins",
								text:
									"Manage users, track platform activity, oversee all data flows, and maintain institutional academic standards.",
							...
							>
								<div
									className={`absolute inset-0 bg-gradient-to-br ${(item as any).gradient} opacity-0 group-hover:opacity-5 transition-opacity`}
								></div>
								<div
									className={`w-16 h-16 mb-6 rounded-xl bg-${(item as any).bg}-100 flex items-center justify-center text-2xl mx-auto group-hover:scale-110 transition-transform`}
								>
									{(item as any).emoji}
								</div>
								<h4
									className={`text-xl font-semibold mb-4 bg-gradient-to-r ${(item as any).gradient} bg-clip-text text-transparent`}
								>
									{(item as any).role}
								</h4>
								<p className="text-blue-900 leading-relaxed">{(item as any).text}</p>
							</div>
						))}
					</div>
				</div>
