...
const AnnouncementPanel = () => {
	...
	useEffect(() => {
		const fetchAssignments = async () => {
			setLoadingAssignments(true); setErrorMsg('');
			try { const data = await api.get('/assignments/filter'); const assignmentsData = Array.isArray(data) ? data : (data && Array.isArray(data.data) ? data.data : []); setAssignments(assignmentsData); setSelected(assignmentsData.length ? assignmentsData[0] : null); } catch (error) { setAssignments([]); setSelected(null); setErrorMsg('Failed to load assignments.'); } finally { setLoadingAssignments(false); }
		};
		fetchAssignments();
	}, []);
	...
	useEffect(() => {
		const fetchAnnouncements = async () => {
			...
			setLoadingAnnouncements(true); setErrorMsg('');
			try { const data = await api.get(`/announcements?course=${selected.course}&semester=${selected.semester}&subject=${selected.subject}`); setAnnouncements(Array.isArray(data) ? data : []); } catch (error) { setAnnouncements([]); setErrorMsg('Failed to load announcements.'); } finally { setLoadingAnnouncements(false); }
		};
		fetchAnnouncements();
	}, [selected]);

	const handleAdd = async (e: React.FormEvent) => {
		e.preventDefault(); if (!newAnnouncement.trim() || !selected) return; setPostingAnnouncement(true); setErrorMsg('');
		try { const payload = { course: selected.course, semester: selected.semester, subject: selected.subject, message: newAnnouncement.trim() }; await api.post('/announcements', payload); setNewAnnouncement(''); const updated = await api.get(`/announcements?course=${selected.course}&semester=${selected.semester}&subject=${selected.subject}`); setAnnouncements(updated.length ? updated : []); } catch (error: any) { alert(error.message || 'Failed to post announcement.'); } finally { setPostingAnnouncement(false); }
	};
					...
					<section className="bg-white rounded-xl shadow p-4">
						...
						{loadingAnnouncements ? (<p>Loading announcements...</p>) : announcements.length === 0 ? (<p className="italic text-gray-500">No announcements yet.</p>) : (
							announcements.map((a, idx) => (
								<article key={idx} className="border border-gray-200 rounded p-4 mb-3 bg-gray-50 shadow-sm" aria-live="polite">
									<p className="text-gray-800 whitespace-pre-wrap">{a.message}</p>
									<p className="text-xs text-gray-500 mt-2">📅 {new Date(a.date || a.createdAt).toLocaleDateString()} — {a.course} Sem {a.semester} — {a.subject}</p>
								</article>
							))
						)}
					</section>
