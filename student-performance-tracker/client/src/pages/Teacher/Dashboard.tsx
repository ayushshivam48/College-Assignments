...
const TeacherDashboard = ({ user }: { user?: any }) => {
	...
	useEffect(() => {
		...
		async function fetchData() {
			try {
				try { const teacherData = await api.get(`/teachers/${user._id}`); setTeacher(teacherData); } catch { setTeacher(user); }
				try { const assignmentsData = await api.get(`/assignments/filter?teacher=${user._id}`); setAssignments(assignmentsData || []); } catch { setAssignments([]); }
				try { setStudents([]); } catch { setStudents([]); }
			} catch { setTeacher(user); setAssignments([]); setStudents([]); }
		}
		fetchData();
	}, [user]);
	...
	useEffect(() => {
		if (!assignments.length) { setTimetable([]); return; }
		async function fetchTimetable() { try { const data = await api.get(`/timetables/filter?role=teacher&teacher=${user._id}`); setTimetable(data || []); } catch { setTimetable([]); } }
		fetchTimetable();
	}, [assignments]);
